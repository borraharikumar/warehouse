...
public class SaleOrderInvoicePdfView extends AbstractPdfView {
	...
	protected void buildPdfDocument(Map<String, Object> model, Document document, PdfWriter writer,
		...
		//Get SaleOrder
				...
				//Get SaleOrderDtl Data
				List<SaleOrderDtl> poDtl = (List<SaleOrderDtl>) model.get("soDtl");
				//Create title
				...
				document.add(title);
				document.add(createMetaData(so, document, totalCost));
				document.add(createOrderDtl(poDtl, document));
				document.add(createFooter(subTotal, cGst, sGst, totalCost));
				document.add(date);
	}
	
	private PdfPTable createMetaData(SaleOrder so, Document document, Double finalCost) throws DocumentException {
		...
		PdfPCell cell1 = new PdfPCell(new Phrase("Customer code", metaDataFont));
		PdfPCell cell2 = new PdfPCell(new Phrase(":   " + so.getCustomer().getUserCode(), metaDataFont));
		PdfPCell cell3 = new PdfPCell(new Phrase("Order code", metaDataFont));
		PdfPCell cell4 = new PdfPCell(new Phrase(":   " + so.getOrderCode(), metaDataFont));
		...
		PdfPCell cell8 = new PdfPCell(new Phrase(":   " + so.getShipmentType().getShipmentCode(), metaDataFont));
	...
	private Element createOrderDtl(List<SaleOrderDtl> poDtl, Document document) {
	...
	private Element createFooter(Double subTotal, Double cGst, Double sGst, Double totalCost) throws DocumentException {
		...
		PdfPCell cell2 = new PdfPCell(new Phrase(":   " + df.format(subTotal)));
		PdfPCell cell3 = new PdfPCell(new Phrase("Central GST", metaDataFont));
		PdfPCell cell4 = new PdfPCell(new Phrase(":   " + df.format(cGst)));
		PdfPCell cell5 = new PdfPCell(new Phrase("State GST", metaDataFont));
		PdfPCell cell6 = new PdfPCell(new Phrase(":   " + df.format(sGst)));
		PdfPCell cell7 = new PdfPCell(new Phrase("Total cost", metaDataFont));
		PdfPCell cell8 = new PdfPCell(new Phrase(":   " + df.format(totalCost)));
		cell1.setBorder(PdfPCell.NO_BORDER);
		cell2.setBorder(PdfPCell.NO_BORDER);
		cell3.setBorder(PdfPCell.NO_BORDER);
		cell4.setBorder(PdfPCell.NO_BORDER);
